...
def update(timestamp,projection,bbox,name_array):
    return_datasets=[]
    add_datasets=[]
    ...
    # WAL allows N readers + 1 writer: serialize the check+insert phase so
    # concurrent pipeline scripts never write at the same time; the whole
    # name array goes in as one transaction / one commit
    with write_lock():
...
def clear():
    ...
    'bufr_vlad_vel1', 'bufr_vlad_vel2', 'bufr_vlad_vel3', 'bufr_vlad_vel4','bufr_vlad_precip', 'bufr_zdr2','bufr_vlad_zdr2','nowcast','TOTAL_PRECIP' ] :
